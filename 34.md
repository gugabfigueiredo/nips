...
```jsonc
{
  ...
  "tags": [
    ...
    ["clone", "<url for git-cloning>", ...], // [DEPRECATED] a url to be given to `git clone` so anyone can clone it
    ["remote", "<url for git remote repository>", ...], // a url to be given to `git remote add` so anyone can clone/pull/push to it
